...
import { Briefcase, Calendar, DollarSign, Building2, Calculator, Save, Info, Loader2, Percent, Store, Layers } from 'lucide-react';
import { toast } from 'sonner';

// Hooks
import { useCompanyConfig } from '../../../hooks/useOrganization';
...
const useSalarySimulation = (control, bcvRate, companyConfig, setValue) => {
    ...
    const calculateBaseFromTotal = (totalValue) => {
        if (companyConfig && totalValue && !isNaN(parseFloat(totalValue))) {
            ...
            if (totalSalary > 0 && splitMode === 'PERCENTAGE') {
    ...
    return {
        ...useMemo(() => {
            ...
            return { totalPackageBs, cestaTicketBs, complementoBs, baseBs, isValid: totalPackageUsd > 0 };
        }, [salaryAmount, baseSalaryBs, bcvRate]),
...
const SalarySimulator = ({ simulation, bcvRate, isLoadingRate, isSubmitting }) => (
    <div className="w-full lg:w-[320px] lg:shrink-0 bg-slate-50/80 backdrop-blur-sm p-6 rounded-[2rem] border border-gray-100/50 flex flex-col shadow-sm lg:self-start lg:sticky lg:top-6">
        ...
        <div className="pt-6 mt-6 border-t border-gray-200/50 flex flex-col gap-3">
            <Button
                type="submit"
                form="contract-form"
                variant="electric"
...
const ContractForm = ({
    register, control, errors, watchedValues,
    handleJobSelected, calculateBaseFromTotal,
    handleSubmit, onSubmit, setValue,
}) => {
    const isJobSelected = !!watchedValues.job_position;

    return (
        <form id="contract-form" className="flex-1 min-w-0 space-y-8" onSubmit={handleSubmit(onSubmit)}>
            ...
            <section>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-wide ml-1 flex items-center gap-2">
                            <Store size={12} className="text-nominix-electric opacity-60" />
                            Sede / Sucursal
                        </label>
                        <Controller
                            name="branch"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wide ml-1 flex items-center gap-2">
                                <Layers size={12} className="text-nominix-electric opacity-60" />
                                Departamento
                            </label>
                            <Controller
                                name="department"
                                ...
                                render={({ field }) => (
                                    <DepartmentSelector
                                        ...
                                        onChange={(val) => {
                                            field.onChange(val);
                                            // Cascade reset al cambiar departamento
                                            setValue('job_position', '');
                                            setValue('position', '');
                                        }}
                                        disabled={!watchedValues.branch}
                                    />
                                )}
                            />
                        </div>
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wide ml-1 flex items-center gap-2">
                                <Briefcase size={12} className="text-nominix-electric opacity-60" />
                                Cargo Estructurado
                            </label>
                            <Controller
                                name="job_position"
                                ...
                                render={({ field }) => (
                                    <JobPositionSelector
                                        ...
                                        departmentId={watchedValues.department}
...
const UpsertContractModal = ({ isOpen, onClose, onSuccess, employeeId, employeeData, contractToEdit = null }) => {
    ...
    const simulation = useSalarySimulation(control, bcvRate, companyConfig, setValue);
    ...
    /**
     * Se ejecuta al seleccionar o crear/editar un cargo.
     * Auto-rellena los campos de salario según la estrategia de la empresa.
     */
    const handleJobSelected = useCallback((job) => {
        if (!job) return;
    ...
    }, [companyConfig, bcvRate, setValue]);

    const onSubmit = async (data) => {
        if (data.contract_type !== 'INDEFINITE' && !data.end_date) {
            toast.error('La fecha de fin es obligatoria para este contrato');
            return;
        }
